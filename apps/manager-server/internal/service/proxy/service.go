...
func New(
	...
	authFileDir := ""
	if len(authFileDirs) > 0 {
		authFileDir = authFileDirs[0]
	}
	return &Service{
...
func (s *Service) proxyToSavedSetup(w http.ResponseWriter, r *http.Request, writeError func(http.ResponseWriter, int, error), useSavedManagementKey bool, rewritePluginOrigin bool) {
	...
	proxy.Director = func(req *http.Request) {
		...
		if ownershipMutation.clearAll || len(ownershipMutation.fileNames) > 0 ||
			shouldEnrichAuthFilesResponse(r, s.authFileDir) {
	...
	enrichAuthFiles := shouldEnrichAuthFilesResponse(r, s.authFileDir)
	proxy.ModifyResponse = func(response *http.Response) error {
		...
		if err := s.restoreInspectionOwnership(
			persistCtx,
			ownershipItemsNotMutated(revokedOwnership, mutation),
		); err != nil {
			return err
		}
		if enrichAuthFiles {
			return s.enrichAuthFilesResponse(response)
		}
		return nil
	}
	proxy.ServeHTTP(w, r)
